  ...
  useEffect(() => {
    setupDatabase().then(database => {
      ...
      if (res.length > 0) {
        ...
        setSchemaInfo(res[0].values.map(v => v[1]).join('\n\n'));
      }
    });
  }, []);
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        try {
          ...
          const schemaRes = db.exec("SELECT sql FROM sqlite_master WHERE type='table'");
          if (schemaRes.length > 0) setSchemaInfo(schemaRes[0].values.map(v => v[0]).join('\n\n'));
